...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test = x_test.view(num_test, -1)
    dists = torch.sqrt(
        torch.sum(x_train ** 2, dim=1, keepdim=True) +
        torch.sum(x_test ** 2, dim=1, keepdim=True).T -
        2 * x_train @ x_test.T
    )
    ##########################################################################
    #                           END OF YOUR CODE                             #
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        idx = torch.topk(dists[:, i], k, largest=False).indices
        y_pred[i] = torch.bincount(y_train[idx]).argmax()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            accuracy = knn_classifier.check_accuracy(x_val_fold, y_val_fold, k, quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    best_k = max(k_to_accuracies, key=lambda k: (sum(k_to_accuracies[k]) / len(k_to_accuracies[k]), -k))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
